    ...
    <div class="cli-modal">
      ...
      <div class="cli-body">
        <div class="cli-row">
          ...
          <input class="cm-input" id="cli-nome" placeholder="Ex: João Silva" value="${(c.nome||'').replace(/"/g,'&quot;')}">
        </div>
        <div class="cli-row">
          <label>Endereço</label>
          <input class="cm-input" id="cli-endereco" placeholder="Rua, número, bairro" value="${(c.endereco||'').replace(/"/g,'&quot;')}">
        </div>
        <div class="cli-grid">
          <div class="cli-row">
            <label>Telefone</label>
            <input class="cm-input" id="cli-telefone" placeholder="(00) 00000-0000" value="${(c.telefone||'').replace(/"/g,'&quot;')}">
          </div>
          <div class="cli-row">
            <label>Plano</label>
            <input class="cm-input" id="cli-plano" placeholder="Ex: 500 MB" value="${(c.plano||'').replace(/"/g,'&quot;')}">
          </div>
        </div>
        <div class="cli-grid">
          <div class="cli-row">
            <label>ONU / Serial</label>
            <input class="cm-input" id="cli-onu" placeholder="HWTC12345678" value="${(c.onuSerial||'').replace(/"/g,'&quot;')}">
          </div>
          <div class="cli-row">
            <label>Status</label>
        ...
        <div class="cli-row">
          ...
          <textarea class="cm-input" id="cli-obs" rows="2" placeholder="Notas internas">${(c.obs||'').replace(/</g,'&lt;')}</textarea>
        </div>
      </div>
...
function renderClientesTable(d){
  ...
  for(let p=0;p<ports;p++){
    ...
    if(c){
      html += `<div class="cli-trow filled" onclick="openClienteModal(findById('${d.id}'),${p},(findById('${d.id}').clientes||[]).find(x=>x.id==='${c.id}'))">
        <span class="cli-port">${pLbl}</span>
        <span class="cli-name"><b>${c.nome}</b>${c.plano?' <span style="color:var(--text3);font-size:9px">'+c.plano+'</span>':''}</span>
        <span class="cli-meta">${c.endereco||'—'}</span>
        <span class="cli-edit">✏️</span>
      </div>`;
    }else{
      html += `<div class="cli-trow empty" onclick="openClienteModal(findById('${d.id}'),${p})">
        <span class="cli-port">${pLbl}</span>
...
// Patcheia openEmModal para injetar a tabela de clientes quando é CTO/Deriv
(function patchEmModal(){
  if(window._clientsPatched)return;
  window._clientsPatched=true;
  const origBuild=window.buildEmTabs;
  // Não temos hook direto — então usamos um observador via MutationObserver no #em-body
  const body=document.getElementById('em-body');
  if(!body)return;
  let lastInjected=null;
  const obs=new MutationObserver(()=>{
    if(!currentEM)return;
    if(currentEM.tipo!=='cto'&&currentEM.tipo!=='deriv')return;
    if(lastInjected===currentEM.id+':'+(currentEM.clientes||[]).length)return;
    // Procura a primeira tab (Propriedades) e injeta antes do .pact
    const tab=body.querySelector('.etc');
    if(!tab)return;
    if(tab.querySelector('.cli-table'))return; // já injetada
    const html=renderClientesTable(currentEM);
    ...
    lastInjected=currentEM.id+':'+(currentEM.clientes||[]).length;
  });
...
function renderClientesPanelList(){
  ...
  filtered.forEach(c=>{
    ...
    html += `<div class="cp-row${inactive?' inactive':''}" onclick="closeClientesPanel();openClienteFromSearch('${pl.id}','${c.id}')">
      <div class="cp-main">
        <div class="cp-name">${inactive?'<span style="color:#9ca3af">○</span> ':'<span style="color:#10B981">●</span> '}<b>${c.nome}</b>${c.plano?' <span style="color:var(--text3);font-size:10px;font-family:var(--mono)">'+c.plano+'</span>':''}</div>
        <div class="cp-meta">${c.endereco?c.endereco+' · ':''}${tlbl} ${pl.nome||pl.id} · porta ${c.porta+1}${c.onuSerial?' · ONU '+c.onuSerial:''}</div>
      </div>
    </div>`;
  });
  el.innerHTML = html;
...
// ═══ Bind: ESC fecha modal de cliente ═══
document.addEventListener('keydown', function(e){
  if(e.key==='Escape'){
    const ov=document.getElementById('cliente-overlay');
    if(ov&&ov.classList.contains('show')){closeClienteModal();}
  }
});
