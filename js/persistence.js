...
function askClear(){
  ...
  showConf('Limpar tudo?','Remove todos os elementos, cabos e conexões.',()=>{
    ...
    if(typeof _indexRebuild==='function')_indexRebuild();
  ...
  try{
    ...
    pms.forEach(pm=>{
      ...
      if(parts.length===1){
        ...
        dd.poly=poly;cabos.push(dd);if(typeof _indexAdd==='function')_indexAdd(dd);
    ...
    if(typeof pruneInvalidFiberConnections==='function')pruneInvalidFiberConnections();
...
function exportGeoJSON(){
  ...
  (mpls||[]).forEach(p=>f.push({type:'Feature',geometry:{type:'Point',coordinates:[p.lng,p.lat]},properties:{tipo:'mpls',id:p.id,nome:p.nome,portas:p.portas}}));
...
async function exportKMZ(){
  ...
  // Escape XML: nomes vindos do usuário podem ter & < > " '
  ...
  armarios.forEach(p=>k+=`<Placemark><name>${xe(p.nome)}</name><Point><coordinates>${p.lng},${p.lat},0</coordinates></Point></Placemark>\n`);
  postes.forEach(p=>k+=`<Placemark><name>${xe(p.nome)}</name><Point><coordinates>${p.lng},${p.lat},0</coordinates></Point></Placemark>\n`);
  olts.forEach(p=>k+=`<Placemark><name>${xe(p.nome)}</name><Point><coordinates>${p.lng},${p.lat},0</coordinates></Point></Placemark>\n`);
  (mpls||[]).forEach(p=>k+=`<Placemark><name>${xe(p.nome)}</name><Point><coordinates>${p.lng},${p.lat},0</coordinates></Point></Placemark>\n`);
  ctos.forEach(p=>k+=`<Placemark><name>${xe(p.nome)}</name><Point><coordinates>${p.lng},${p.lat},0</coordinates></Point></Placemark>\n`);
  emendas.forEach(p=>k+=`<Placemark><name>${xe(p.nome)}</name><Point><coordinates>${p.lng},${p.lat},0</coordinates></Point></Placemark>\n`);
  derivs.forEach(p=>k+=`<Placemark><name>${xe(p.nome)}</name><Point><coordinates>${p.lng},${p.lat},0</coordinates></Point></Placemark>\n`);
  cabos.forEach(c=>{const co=c.points.map(p=>`${p[1]},${p[0]},0`).join('\n');const rgb=c.color.replace('#','');const r=rgb.slice(0,2),g=rgb.slice(2,4),b=rgb.slice(4,6);k+=`<Placemark><name>${xe(c.nome)}</name><Style><LineStyle><color>ff${b}${g}${r}</color><width>3</width></LineStyle></Style><LineString><tessellate>1</tessellate><coordinates>${co}</coordinates></LineString></Placemark>\n`;});
  k+=`</Document></kml>`;
...
function buildProjectSnapshot(){
  return {
    version:8.4,
    counters:{pcnt,ctocnt,emcnt,dcnt,cabocnt,oltcnt,armcnt,mplscnt},
    ...
    mpls:mpls.map(m=>({id:m.id,num:m.num,lat:m.lat,lng:m.lng,tipo:m.tipo,nome:m.nome,portas:m.portas,parentId:m.parentId,obs:m.obs,fibers:m.fibers||{connections:[],diagramPositions:{}}})),
    ...
    cabos:cabos.map(c=>({id:c.id,tipo:c.tipo,points:c.points,grupos:c.grupos,fpg:c.fpg,total:c.total,estilo:c.estilo,color:c.color,typeId:c.typeId,typeName:c.typeName,tubeType:c.tubeType||'loose',dist:c.dist,nome:c.nome,obs:c.obs,anchors:c.anchors,anchorMap:c.anchorMap,fibers:c.fibers,fiberSegments:c.fiberSegments||{}})),
...
function saveProjectToLocal(){
  try{
    ...
    // localStorage síncrono: prova de gravação imediata (até 5MB).
    let lsOk=false;
    try{
      const str=JSON.stringify(snap);
      if(str.length<4500000){localStorage.setItem(PROJECT_STORAGE_KEY,str);lsOk=true;}
    }catch(e){/* over-quota: depende do IDB */}
    // IndexedDB assíncrono: sem limite, mas pode falhar (privado/quota). Propaga toast.
    saveProjectToIDB(snap).then(()=>{
      setTimeout(()=>showSavingIndicator(false),300);
    }).catch(err=>{
      console.warn('IDB save falhou:',err);
      if(!lsOk){
        showToast('⚠ Falha ao salvar projeto: '+(err.message||err),'error');
        const el=document.getElementById('sb-autosave');
        if(el){el.textContent='● Falha ao salvar';el.style.color='var(--red,#EF4444)';}
      }else{
        setTimeout(()=>showSavingIndicator(false),300);
      }
    });
...
// Flush síncrono antes do unload: garante que último estado vai pro localStorage.
...
async function doAutoBackup(){
  try{
    ...
    const snap=buildProjectSnapshot();
    const stats={
      elements:(armarios.length+postes.length+ctos.length+emendas.length+derivs.length+olts.length+(mpls||[]).length),
      cables:cabos.length,
...
let _projectLoadInProgress=false;
// window._projectLoading: true enquanto carregamento async do IDB roda.
// scheduleAutosave consulta este flag para não sobrescrever IDB com dados
// parciais antes da hidratação terminar.
window._projectLoading=true;
function loadProjectFromLocal(){
  ...
  // Flag _projectLoadInProgress evita dupla-aplicação se o usuário disparar reload
  // antes do async terminar.
  setTimeout(async()=>{
    if(_projectLoadInProgress){window._projectLoading=false;return;}
    ...
    finally{_projectLoadInProgress=false;window._projectLoading=false;}
  },500);
  try{
    ...
    connections.forEach(c=>map.removeLayer(c.line));connections=[];
    cleanupCabo();
    ctocnt=snap.counters.ctocnt;emcnt=snap.counters.emcnt;dcnt=snap.counters.dcnt;cabocnt=snap.counters.cabocnt;oltcnt=snap.counters.oltcnt;mplscnt=snap.counters.mplscnt||0;
    ...
    (snap.mpls||[]).forEach(m=>{mplscnt=Math.max(mplscnt,m.num||0);const d={...m,fibers:m.fibers||{connections:[],diagramPositions:{}}};mpls.push(d);if(d.parentId){const parent=armarios.find(p=>p.id===d.parentId);if(parent){if(!parent.children)parent.children=[];parent.children.push({tipo:'mpls',id:d.id});refreshArmarioIcon(parent);}}});
