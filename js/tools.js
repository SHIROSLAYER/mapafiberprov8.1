...
function addMeasurePoint(lat,lng){
  ...
  if(measurePoints.length>=2){
    ...
    // Label de segmento
    const i=measurePoints.length-1;
    const segDist=L.latLng(measurePoints[i-1]).distanceTo(L.latLng(measurePoints[i]));
    const mid=[(measurePoints[i-1][0]+measurePoints[i][0])/2,(measurePoints[i-1][1]+measurePoints[i][1])/2];
    const lbl=L.marker(mid,{icon:L.divIcon({className:'measure-label',html:fmtDist(segDist),iconAnchor:[20,10]})}).addTo(map);
    measureLabels.push(lbl);
  }
  // Marcador de ponto
  const dot=L.circleMarker([lat,lng],{radius:5,color:'#F59E0B',fillColor:'#fff',fillOpacity:1,weight:2}).addTo(map);
  measureLabels.push(dot);
  showMeasureTotal();
}
function showMeasureTotal(){
  if(measurePoints.length<2){document.getElementById('measure-total').classList.remove('show');return;}
  let d=0;for(let i=1;i<measurePoints.length;i++) d+=L.latLng(measurePoints[i-1]).distanceTo(L.latLng(measurePoints[i]));
  const el=document.getElementById('measure-total');
  el.textContent=`📏 Total: ${fmtDist(d)}  (${measurePoints.length} pontos)`;
...
function finishMeasure(){
  if(measurePreviewLine){map.removeLayer(measurePreviewLine);measurePreviewLine=null;}
  showMeasureTotal();
  if(measurePolyline) measurePolyline.setStyle({opacity:1});
  showToast(`📏 ${document.getElementById('measure-total').textContent.replace('📏 ','')}`,'success');
  setMode('select');
}
...
function parseAndPreviewCSV(text){
  ...
  document.getElementById('csv-summary').innerHTML=
    `<span class="csv-badge ok">✓ ${ok} prontos para importar</span>`+
    (err?`<span class="csv-badge err">✗ ${err} com erro</span>`:'')+
    `<span style="color:var(--text3);font-size:10px;margin-left:auto">Total: ${rows.length} linhas lidas</span>`;
  ...
  document.getElementById('csv-tbody').innerHTML=rows.slice(0,200).map(r=>{
    ...
    return `<tr><td class="${r._errs.length?'err':''}">${r._row}</td>
      <td class="${r._errs.length?'err':r.tipo==='armario'?'warn':'ok'}">${r.tipo}</td>
      <td>${r.nome||'—'}</td><td>${r.lat}</td><td>${r.lng}</td>
      <td style="max-width:120px;overflow:hidden;text-overflow:ellipsis">${r.obs||'—'}</td>${st}</tr>`;
  }).join('');
