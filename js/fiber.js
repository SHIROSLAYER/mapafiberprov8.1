...
function buildCablePanel(c){
  ...
  if(c.grupos>1){
    var FH=22,TH=14;
    var totalH=c.grupos*(TH+c.fpg*FH)+16;
    div.className='panel cv-panel';
    div.style.cssText='width:100px;display:flex;flex-direction:'+(isE?'row':'row-reverse')+';min-height:'+totalH+'px;border-radius:8px;overflow:hidden;';
    div.appendChild(buildLbar());
    var bdy2=document.createElement('div');
    bdy2.style.cssText='width:80px;flex-shrink:0;position:relative;background:#f8fafc;';
    var tb2=document.createElement('div');
    tb2.style.cssText='position:absolute;top:0;bottom:0;width:10px;background:#808080;border:1px solid #555;z-index:0;'+(isE?'left:4px':'right:4px');
    ...
    for(var g=0;g<c.grupos;g++){
      ...
      for(var f2=0;f2<c.fpg;f2++){
        ...
        row2.style.cssText='display:flex;align-items:center;height:'+FH+'px;position:relative;z-index:1;gap:2px;padding:0 3px;'+(isE?'justify-content:flex-end;':'justify-content:flex-start;');
        ...
        arw2.setAttribute('width','11');arw2.setAttribute('height','15');
  ...
  // ── Tubo único (1 grupo) — visual limpo, só faixa do nome do cabo ──
  var FH=26;
  var panelH=c.fpg*FH+16;
  div.className='panel cv-panel';
  div.style.cssText='width:100px;display:flex;flex-direction:'+(isE?'row':'row-reverse')+';min-height:'+panelH+'px;border-radius:8px;overflow:hidden;';
  div.appendChild(buildLbar());
  var bdy=document.createElement('div');
  bdy.style.cssText='width:80px;flex-shrink:0;position:relative;background:#f8fafc;';
  var tb=document.createElement('div');
  tb.style.cssText='position:absolute;top:0;bottom:0;width:10px;background:#808080;border:1px solid #555;z-index:0;'+(isE?'left:4px':'right:4px');
  bdy.appendChild(tb);
  for(var f=0;f<c.fpg;f++){
    var fi=f,fc=ABNT[f%12];
    var row=document.createElement('div');
    row.style.cssText='display:flex;align-items:center;height:'+FH+'px;position:relative;z-index:1;gap:2px;padding:0 3px;'+(isE?'justify-content:flex-end;':'justify-content:flex-start;');
