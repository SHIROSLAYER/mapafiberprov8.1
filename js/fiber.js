...
.cpath:hover{stroke:#EF4444;opacity:1;stroke-width:3.6}
...
.fdot{width:36px;height:26px;cursor:crosshair;transition:transform .12s,filter .12s;flex-shrink:0;display:block;overflow:visible;position:relative;z-index:1}
...
.fdot.conn .fhx{stroke:#3B82F6;stroke-width:2.5;filter:drop-shadow(0 0 3px rgba(59,130,246,.5))}
...
.cto-bar{width:100%;border-top:2px solid #e5e7eb;padding:6px 8px;display:flex;flex-wrap:wrap;gap:3px;justify-content:center}
.cto-bar-lbl{width:100%;font-size:8px;color:#9ca3af;font-family:monospace;text-align:center;margin-bottom:2px}
...
function buildCablePanel(c){
  ...
  if(c.grupos>1){
    ...
    for(var g=0;g<c.grupos;g++){
      ...
      for(var f2=0;f2<c.fpg;f2++){
        ...
        var dot2=makeDot(pid,fi2,fc2.hex,f2+1,escH(c.nome||c.id)+' T'+(g+1)+' F'+(f2+1));
  ...
  for(var f=0;f<c.fpg;f++){
    ...
    var dot=makeDot(pid,fi,fc.hex,f+1,escH(c.nome||c.id)+' F'+(f+1));
...
function buildChildPanel(s){
  ...
  var snome=s.nome&&s.nome.length<26?s.nome:(s.tipo.toUpperCase()+' '+s.id.slice(-4));
  ...
  if(s.tipo==='emenda'){
    ...
    var inDot=makeDot('in:'+s.id,0,sc,'▶',escH(snome)+' entrada');
    ...
    for(var p=0;p<ports;p++){
      var pd=makeDot('out:'+s.id,p,sc,p+1,escH(snome)+' porta '+(p+1));
...
function makeDot(eid,fi,color,label,title){
  ...
  p.setAttribute('d','M10 1 L30 1 L39 15 L30 29 L10 29 L1 15 Z');
  p.setAttribute('fill',color);
  svg.appendChild(p);
  var t=document.createElementNS(SVG_NS,'text');
  t.setAttribute('x','20');t.setAttribute('y','15');
...
function placeAll(){
  ...
  ELEM.children.forEach(function(s){
    ...
    inner.appendChild(p);panelDom[pid]=p;bindPanelDrag(p,pid);
    var ports=s.portas||s.capacidade||8;
    var h=s.tipo==='emenda'?100:Math.ceil(ports/4)*26+80;
    yC+=h+GAP;
  });
  applyPan(0,0);
}
...
function bindFiberDot(dot){
  ...
  dot.addEventListener('touchend',function(e){
    ...
    finishFiber(tgt&&tgt.classList.contains('fdot')?tgt:null);
  },{passive:false});
}
...
document.addEventListener('mouseup',function(e){
  ...
  if(fdrag){var t=document.elementFromPoint(e.clientX,e.clientY);finishFiber(t&&t.classList.contains('fdot')?t:null);return;}
...
function movePanel(drag,cx,cy){
  ...
  var x=Math.max(0,cx-ir.left-drag.ox);
  var y=Math.max(0,cy-ir.top-drag.oy);
  drag.panel.style.left=x+'px';drag.panel.style.top=y+'px';
  positions[drag.pid]={x:x,y:y};drawSVG();
}
...
function finishFiber(targetDot){
  ...
  if(ex>=0){connections.splice(ex,1);sbMsg('Conexão removida');}
  else{connections.push({fromId:src.eid,fromFi:src.fi,toId:tEid,toFi:tFi});sbMsg('✓ Fibras conectadas!');}
  markUnsaved();updateAll();
}
...
function drawSVG(){
  ...
  svg.querySelectorAll('.cpath').forEach(function(p){p.remove();});
  var prev=document.getElementById('prev-path');
  connections.forEach(function(conn,i){
    ...
    path.setAttribute('class','cpath');
    path.setAttribute('d','M'+fp.x+','+fp.y+' C'+mx+','+fp.y+' '+mx+','+tp.y+' '+tp.x+','+tp.y);
    path.setAttribute('marker-end','url(#arr)');
    path.addEventListener('click',(function(idx){return function(e){e.stopPropagation();connections.splice(idx,1);markUnsaved();updateAll();sbMsg('Conexão removida');};  })(i));
...
function quickAdd(tipo,parentElem){
  ...
  if(tipo==='emenda'){var nd=addSubElement('emenda',parentElem);map.setView([nd.lat,nd.lng],18);showToast('Emenda '+(nd.nome||nd.id)+' criada.');if(currentEM&&currentEM.id===parentElem.id)openEmModal(parentElem);return;}
