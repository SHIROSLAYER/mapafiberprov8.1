...
.fdot.remote-conn .fhx{stroke:#06B6D4;stroke-width:2.8;filter:drop-shadow(0 0 4px rgba(6,182,212,.7));stroke-dasharray:2 1.5}
...
function updateDots(){
  document.querySelectorAll('.fdot').forEach(function(dot){
    ...
    var remote=isRemoteConn(eid,fi);
    dot.classList.toggle('conn',local);
    dot.classList.toggle('remote-conn',remote&&!local);
    // Tooltip enriquecido pra conexão remota
    if(remote&&!local){
      var owner=getRemoteConnOwner(eid,fi);
      dot.dataset.dotTitleRemote=' (conectada em '+owner+')';
    }else{
