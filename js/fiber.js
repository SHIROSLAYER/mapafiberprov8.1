...
/* Fibra "passando" — não tem conexão local mas carrega sinal de OLT/MPLS upstream */
.fdot.upstream-olt .fhx{stroke:#0d6b50;stroke-width:1.6;stroke-dasharray:3 2}
.fdot.upstream-mpls .fhx{stroke:#EF4444;stroke-width:1.8;stroke-dasharray:3 2}
.fdot.upstream-olt .fcheck circle{fill:#10B981!important}
.fdot.upstream-mpls .fcheck circle{fill:#EF4444!important}
.fdot.upstream-olt .fcheck,.fdot.upstream-mpls .fcheck{opacity:1}
...
function getOLTInfoForChild(s){
  // Busca a conexão local da entrada do CTO/Deriv → cable e rastreia até a OLT
  var entryEid=(s.tipo==='olt')?null:('in:'+s.id);
  ...
  for(var i=0;i<connections.length;i++){
    ...
    try{
      if(window.opener&&typeof window.opener.findOLTForFiber==='function'){
        var info=window.opener.findOLTForFiber(cabId,peer.fi);
        if(info)return info;
      }
    }catch(_){}
  }
...
function buildChildPanel(s){
  ...
  if(!noInput){
    ...
    var oltInfo=getOLTInfoForChild(s);
    sigEl.style.cssText='font-size:7px;font-family:monospace;font-weight:700;color:#0d6b50;background:#dcfce7;border:1px solid #86efac;border-radius:4px;padding:2px 4px;margin:0 0 4px 0;align-self:flex-end;max-width:62px;text-align:center;line-height:1.2;white-space:normal;word-break:break-all;';
    sigEl.textContent=oltInfo?(oltInfo.nome+'·PON'+oltInfo.pon):'N/Inform.';
    sigEl.title='Sinal Esperado: '+(oltInfo?(oltInfo.nome+' PON-'+oltInfo.pon):'Não informado');
...
function drawSVG(){
  ...
  connections.forEach(function(conn,i){
    ...
    // Conexões envolvendo MPLS (direto ou via upstream) são desenhadas em vermelho
    var fIsMPLS=isMPLSEndpoint(conn.fromId)||fe.classList.contains('upstream-mpls');
    var tIsMPLS=isMPLSEndpoint(conn.toId)||te.classList.contains('upstream-mpls');
...
function updateDots(){
  // .conn = conexão LOCAL (sutil + check verde)
  // .upstream-olt / .upstream-mpls = fibra de cabo de passagem que rastreia
  // até OLT/MPLS em outro elemento — mostra check colorido sem ser linha local
  document.querySelectorAll('.fdot').forEach(function(dot){
    ...
    // Para fibras de cabo SEM conexão local, verifica origem global (OLT/MPLS)
    if(!local&&eid.indexOf('cable:')===0){
      try{
        if(window.opener&&typeof window.opener.findSourceForFiber==='function'){
          ...
          if(src){
            ...
            dot.dataset.dotTitleUpstream=src.tipo.toUpperCase()+' '+src.nome+' · '+(src.tipo==='olt'?'PON ':'fibra ')+src.pon;
          }
        }
      }catch(_){}
    }
...
function showTip(dot,cx,cy){
  ...
  // Rastreia origem do sinal (OLT/MPLS) — funciona se a fibra tem conexão local
  // OU se o dot está marcado como upstream (fibra de passagem rastreável).
  ...
  if(sourceInfo){
    ...
    var tipoLbl=isMplsSrc?'MPLS':'OLT';
    var fibLbl=isMplsSrc?'fibra ':'PON ';
    lines.push('<div style="margin-top:6px;padding:8px 10px;background:'+grad+';border-radius:6px;color:#fff;font-weight:700;font-size:11px;line-height:1.35;box-shadow:0 2px 8px '+shad+'"><span style="font-size:14px">💡</span> Sinal de <b style="font-family:monospace">'+escH(tipoLbl+' '+sourceInfo.nome)+'</b><br><span style="font-size:10px;font-weight:600;opacity:.95">'+fibLbl+sourceInfo.pon+'</span></div>');
  }
