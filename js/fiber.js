...
function buildChildPanel(s){
  // CTO=verde (como foto), Deriv=roxo, OLT=ciano, Emenda=vermelho
  var COL={cto:'#10B981',emenda:'#EF4444',deriv:'#8B5CF6',olt:'#06B6D4'};
  ...
  // CTO/Deriv/OLT: painel vertical estreito (estilo cabo)
  var ports=s.portas||s.capacidade||8;
  var isOLT=s.tipo==='olt';
  var FH=22;
  var infoH=20; // altura do header de placa/PON
  var totalH=ports*FH+infoH+12;
  // Para OLT: calcula uso atual e mostra X/Y no label
  var oltUsage=null;
  if(isOLT){
    try{
  ...
  var portasLbl=isOLT?(oltUsage?(oltUsage.used+'/'+oltUsage.total+' PON'):(ports+' PON')):(ports+' portas');
  ...
  // Para CTO/Deriv: input externo colado à esquerda da barra
  if(!isOLT){
  ...
  // Header pequeno indicando "PONs" ou "SAÍDAS"
  ...
  bhdr.textContent=isOLT?'PONs':'SAÍDAS';
...
function updateDots(){
  // ISOLAMENTO VISUAL: marca .conn (sutil + check) só para conexões LOCAIS.
  // Brilho .lit é gerenciado por selectConn — só acende quando há conexão selecionada.
...
function showTip(dot,cx,cy){
  ...
  // Rastreia OLT APENAS se essa fibra tem alguma conexão (local ou remota).
  // Sem conexão = fibra órfã, não deve mostrar info de OLT mesmo que outra
  // fibra da mesma cable id/fi tenha algum vínculo (evita falso positivo).
  var oltInfo=null;
  if(eid.indexOf('cable:')===0&&(localConn||remoteConn)){
    var cabId=eid.slice(6).replace('::I','').replace('::O','');
    try{
      if(window.opener&&window.opener.findOLTForFiber){
  ...
  if(oltInfo){
    lines.push('<div style="margin-top:6px;padding:8px 10px;background:linear-gradient(135deg,#06b6d4,#0d9488);border-radius:6px;color:#fff;font-weight:700;font-size:11px;line-height:1.35;box-shadow:0 2px 8px rgba(6,182,212,.5)"><span style="font-size:14px">💡</span> Sinal de <b style="font-family:monospace">'+escH(oltInfo.nome)+'</b><br><span style="font-size:10px;font-weight:600;opacity:.95">PON '+oltInfo.pon+'</span></div>');
  }
  if(!localConn&&!remoteConn&&!oltInfo){
    lines.push('<span style="color:#9ca3af">Dispon\xEDvel — arraste para conectar</span>');
  }else if(!localConn&&(remoteConn||oltInfo)){
...
function _captureDiagram(){
  // Calcula bounding box de todos painéis em coordenadas mundo (pré-transform)
  var inner=document.getElementById('inner');
  ...
  // html2canvas captura o elemento; salvamos transform/scale e ajustamos
