...
function buildEmTabs(d){
  ...
  // ═══ Aba FIBRAS (apenas para cabos) ═══
  if(d.tipo==='cabo'){
    const grupos=d.grupos||1, fpg=d.fpg||12, total=d.total||(grupos*fpg);
    let fh=`<div class="fibers-tab">
      <div class="ft-hdr">
        <div class="ft-col ft-col-num">#</div>
        <div class="ft-col ft-col-grp">Grupo · Fibra</div>
        <div class="ft-col ft-col-color">Cor</div>
        <div class="ft-col ft-col-olt">OLT conectada</div>
      </div>
      <div class="ft-list" id="ft-list-${d.id}">`;
    for(let i=0;i<total;i++){
      const a=abntColor(i,fpg);
      const olt=findOLTForFiber(d.id,i);
      const oltCell=olt?`<span style="color:var(--cyan);font-weight:600">OLT ${escH(olt.nome)}</span> <span style="color:var(--text3);font-size:10px">PON ${olt.pon}</span>`:`<span style="color:var(--text3);font-style:italic">— não conectada</span>`;
      const grpLabel=grupos>1?`T${a.tubeN} · F${a.fiberN}`:`F${a.fiberN}`;
      fh+=`<div class="ft-row" data-fi="${i}" onclick="selectFiberRow(this,'${d.id}',${i})">
        <div class="ft-col ft-col-num">${i+1}</div>
        <div class="ft-col ft-col-grp">${grpLabel}</div>
        <div class="ft-col ft-col-color"><span class="ft-swatch" style="background:${a.fiber.hex};border-color:${a.tube.hex}"></span><span class="ft-color-name">${a.fiber.name}</span></div>
        <div class="ft-col ft-col-olt">${oltCell}</div>
      </div>`;
    }
    fh+=`</div>
      <div class="ft-actions" id="ft-actions-${d.id}" style="display:none">
        <button class="pab gn" onclick="onShowSelectedFiberPath('${d.id}')">▸ Mostrar caminho completo da fibra</button>
      </div>
    </div>`;
...
function selectFiberRow(rowEl,cabId,fi){
  // Limpa seleção anterior na lista
  var list=document.getElementById('ft-list-'+cabId);
  if(list){list.querySelectorAll('.ft-row').forEach(function(r){r.classList.remove('sel');});}
