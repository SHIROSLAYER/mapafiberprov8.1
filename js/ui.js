...
function updList(){
  ...
  if(armarios.length){
    ...
    armarios.forEach(a=>{
      ...
      const aRow=_treeRow(aId,0,'#EC4899','🗄',a.nome||a.id,a.id,'Armário',childOlts.length?childOlts.length+' OLT':null,childOlts.length>0);
      h+=aRow.html;
      if(aRow.exp){
        childOlts.forEach(o=>{
          const oId='o:'+o.id;
...
function performSearch() {
  ...
  const results = [];
  const COR = {armario:'#EC4899',poste:'#3B82F6',cto:'#F59E0B',emenda:'#EF4444',deriv:'#8B5CF6',cabo:'#1D9E75',olt:'#06B6D4'};
  // Elementos
