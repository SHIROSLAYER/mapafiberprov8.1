...
function _treeRow(uid,depth,color,iconChar,name,refId,tag,meta,canExpand,opts){
  ...
  const click=refId?`onclick="zoomTo('${refId}')" ondblclick="openFromList('${refId}')"`:'';
  const del=(refId&&!opts.noDel)?`<span class="eidel" onclick="event.stopPropagation();askRemoveById('${refId}')">✕</span>`:'';
  const chevAttr=canExpand?`onclick="event.stopPropagation();toggleTreeNode('${uid}')"`:'';
  const tagHtml=tag?`<span class="ei-tag" style="color:${color}">${tag}</span>`:'';
  const metaHtml=meta?`<span class="ei-meta">${meta}</span>`:'';
  return {html:`<div class="ei ei-tree" ${click} style="padding-left:${depth*14+6}px;border-left:3px solid ${color}55">`+
    ...
    `<span class="ein">${name}${tagHtml}${metaHtml}</span>${del}</div>`,
    exp:exp};
}
        ...
        childOlts.forEach(o=>{
          ...
          if(oRow.exp){
            for(let pi=0;pi<pons;pi++){
              ...
              const ponMeta=conn?`<span style="color:var(--green)">→ ${conn.cabId}</span>`:'<span style="color:var(--text3)">livre</span>';
              ...
              if(pRow.exp&&conn){
                ...
                if(typeof clientesNoCaminho==='function'){
                  ...
                  clients.forEach(cc=>{
                    const cliId='cli:'+cc.cliente.id+':'+pId;
                    const meta=(cc.cliente.plano||'')+(cc.cliente.endereco?' · '+cc.cliente.endereco.slice(0,20):'');
                    h+='<div class="ei ei-tree" style="padding-left:'+(3*14+6)+'px;border-left:3px solid #10B98155;cursor:pointer" onclick="openClienteModal(findById(\''+cc.cto.id+'\'),'+cc.porta+',(findById(\''+cc.cto.id+'\').clientes||[]).find(x=>x.id===\''+cc.cliente.id+'\'))">'+
                      '<span class="ei-chev"></span>'+
                      '<span class="ei-icon" style="background:#10B981;color:#fff">👤</span>'+
                      '<span class="ein"><b>'+cc.cliente.nome+'</b>'+
                      ' <span class="ei-tag" style="color:#10B981">Cliente</span>'+
                      (meta?' <span class="ei-meta">'+meta+'</span>':'')+
                      '</span></div>';
                  });
                }
              }
...
function findById(id){return postes.find(p=>p.id===id)||ctos.find(p=>p.id===id)||emendas.find(p=>p.id===id)||derivs.find(p=>p.id===id)||cabos.find(p=>p.id===id)||olts.find(p=>p.id===id)||(typeof mpls!=='undefined'?mpls.find(p=>p.id===id):null)||armarios.find(p=>p.id===id);}
...
function performSearch() {
  ...
  results.slice(0, 25).forEach(r => {
    if (r.type === 'cliente') {
      ...
      html += `<div class="search-result" onclick="openClienteFromSearch('${r.el.id}','${r.cliente.id}')">
        <span style="color:#10B981;font-weight:700">👤 ${r.label}</span>
        <span style="color:var(--text3);font-size:10px;font-family:var(--mono);display:block;margin-top:2px">${ctolbl} · porta ${r.cliente.porta+1}${r.meta?' · '+r.meta:''}</span></div>`;
    } else {
      const cor = COR[r.el.tipo] || '#888';
      html += `<div class="search-result" onclick="zoomAndSelect('${r.el.id}')">
        <span style="color:${cor};font-weight:700">${r.meta}</span> · ${r.label}
        ${r.el.obs?`<span style="color:var(--text3);font-size:10px;font-family:var(--mono);display:block;margin-top:2px">${r.el.obs.slice(0,60)}</span>`:''}
      </div>`;
    }
  });
