    ...
    <div class="help-modal">
      ...
      <div class="help-body">
        ...
        <section>
          ...
          <div class="kb"><kbd>Enter</kbd> Finalizar cabo</div>
...
function renderAlertsBanner(alerts){
  ...
  bn.innerHTML=`<span class="alerts-icon">${hi>0?'⛔':'⚠️'}</span>
    ...
    <span class="alerts-summary">${alerts[0].texto}${alerts.length>1?` <span style="opacity:.7">(+${alerts.length-1} mais)</span>`:''}</span>
...
function showAlertsList(alerts){
  ...
  const rows=alerts.map(a=>{
    const click=a.id?`onclick="zoomTo('${a.id}');document.getElementById('alerts-overlay').classList.remove('show')"`:'';
    return `<div class="al-row al-${a.tipo}" ${click}><span>${a.tipo==='error'?'⛔':'⚠️'}</span><span>${a.texto}</span>${a.id?'<span class="al-arrow">→</span>':''}</div>`;
  }).join('');
  ov.innerHTML=`<div class="help-modal" style="max-width:520px"><div class="help-hdr"><h2>⚠️ Alertas (${alerts.length})</h2><button class="cc-close" onclick="document.getElementById('alerts-overlay').classList.remove('show')">✕</button></div><div class="help-body">${rows}</div></div>`;
  ov.classList.add('show');
}
...
// Registra em window (capture) — recebe primeiro que document e que outros handlers
window.addEventListener('keydown', _featKeyOnce, true);

// Recalcula alertas periodicamente
setInterval(()=>{if(!READONLY)checkAlerts();},15000);
