...
function confirmCableModal(){
  ...
  const tubeType=(document.getElementById('cm-tubetype')||{}).value||'loose';
  pendingCable={id:idVal,grupos,fpg,total:grupos*fpg,estilo,color,typeId:selectedCableTypeId,typeName:ct.name,tubeType,anchors:[]};
  document.getElementById('cable-overlay').classList.remove('show');
...
function highlightAnchorsForCabo(on){
  anchorables().forEach(a=>{
    ...
    if(on){
      // Suprime apenas o handler normal — preserva o handler do bindDblTap.
      if(a._normalClick)a.marker.off('click',a._normalClick);
      if(!a._anchorHandler){
      ...
      if(a._anchorHandler){a.marker.off('click',a._anchorHandler);}
      // Restaura o handler normal (não cria um novo — evita listeners duplicados).
      if(a._normalClick)a.marker.on('click',a._normalClick);
    }
  });
}
...
function onMapClick(e){
  ...
  // Em modo poste/armário, um dblclick rápido dispara dois clicks — suprime o segundo se vier em ≤300ms.
  if(mode==='armario'||mode==='poste'){
    const now=Date.now();
    if(window._lastAddClick&&now-window._lastAddClick<300)return;
    window._lastAddClick=now;
  }
...
function finishCabo(){
  ...
  // pushState ANTES de criar o cabo — captura estado anterior para undo funcionar.
  stateManager.pushState(`Criar Cabo ${pendingCable.id}`);
  // cabocnt já foi incrementado em confirmCableModal (se o usuário não digitou
  // um ID custom). Não incrementar de novo aqui — causava saltos na numeração.
  ...
  const safeId = (typeof uniqueId==='function')?uniqueId(pendingCable.id):pendingCable.id;
  const safeNome = (typeof uniqueName==='function')?uniqueName(safeId):safeId;
  const d = {
    id: safeId, tipo:'cabo', points: [...simplified],
    ...
    tubeType: pendingCable.tubeType||'loose',
    dist: calcDist(simplified), nome: safeNome, obs: '',
  ...
  d.poly = poly; cabos.push(d); if(typeof _indexAdd==='function')_indexAdd(d);
...
function showCablePopup(d,latlng){
  ...
  const accent=d.color||'#1D9E75';
  const attColor=att.margin>=5?'#10B981':att.margin>=0?'#F59E0B':'#EF4444';
  // Usa mesmo wrapper visual do popup de elemento (.elem-tip-wrap = fundo escuro)
  const html=`<div class="elem-tip-wrap">
    <div class="et-hdr" style="background:linear-gradient(135deg,${accent}33,transparent);border-left:4px solid ${accent}">
      <div class="et-tag" style="color:${accent}">Cabo</div>
      <div class="et-name">${d.nome||d.id}</div>
    </div>
    <div class="et-body">
      <div class="et-row" style="border-left-color:${accent}"><span class="et-dot" style="background:${accent}"></span><span class="et-lbl" style="color:${accent}">Tipo</span><span class="et-vl">${d.typeName||d.typeId||'—'}</span></div>
      <div class="et-row" style="border-left-color:#9CA3AF"><span class="et-dot" style="background:#9CA3AF"></span><span class="et-lbl" style="color:#9CA3AF">FO</span><span class="et-vl">${d.grupos||1}T × ${d.fpg||12} = ${d.total||(d.grupos||1)*(d.fpg||12)}FO</span></div>
      <div class="et-row" style="border-left-color:#10B981"><span class="et-dot" style="background:#10B981"></span><span class="et-lbl" style="color:#10B981">Dist.</span><span class="et-vl">${distFmt}</span></div>
      <div class="et-row" style="border-left-color:${attColor}"><span class="et-dot" style="background:${attColor}"></span><span class="et-lbl" style="color:${attColor}">Atenuação</span><span class="et-vl">${att.tot.toFixed(2)} dB / ${ATT.budget} dB</span></div>
      ${anchorNames?`<div class="et-meta">Ancoragens: ${anchorNames}${anchorIds.length>3?' …':''}</div>`:''}
    </div>
  </div>`;
  L.popup({closeButton:false,className:'elem-popup',autoPan:false,offset:[0,-4],maxWidth:320})
...
function showCableChooser(list,latlng){
  ...
  list.forEach(function(c){
    ...
    rows+='<div class="cc-row" onclick="pickCable(\''+escJs(c.id)+'\')">'+
      '<span class="cc-swatch" style="background:'+c.color+'"></span>'+
      '<div class="cc-info"><b>'+escH(c.nome||c.id)+'</b>'+
      '<span class="cc-meta">'+escH(c.typeName||c.typeId||'—')+' · '+(c.total||'?')+'FO · '+escH(distFmt)+'</span>'+
      '<span class="cc-anchors">'+escH(anchors)+'</span></div>'+
...
function traceFiberGlobal(startEid,startFi){
  ...
  var safety=0;
  var SAFETY_MAX=2000; // alguns projetos grandes precisam de mais; antes era 200 silencioso.
  while(queue.length&&safety++<SAFETY_MAX){
    var cur=queue.shift();
    ...
    // Atravessa CTO/Derivação: input ↔ outputs (todos compartilham mesma origem OLT
    // via splitter). Permite que o rastreio passe por elementos intermediários e
    // que CTOs downstream identifiquem corretamente a OLT de origem.
    if(cur.eid.indexOf('in:')===0||cur.eid.indexOf('out:')===0){
      var sid=cur.eid.replace(/^(in|out):/,'');
      var el=findById(sid);
      if(el&&(el.tipo==='cto'||el.tipo==='deriv')){
        var ports=el.portas||el.capacidade||8;
        if(cur.eid.indexOf('in:')===0){
          // De entrada → todas as saídas
          for(var pp=0;pp<ports;pp++){
            var oeid='out:'+sid;
            var okey=oeid+':'+pp;
            if(!visited[okey]){
              visited[okey]=true;
              hops.push({eid:oeid,fi:pp,viaSplitter:true});
              queue.push({eid:oeid,fi:pp});
            }
          }
        }else{
          // De saída → entrada (volta pra montante até chegar na OLT)
          var ieid='in:'+sid, ikey=ieid+':0';
          if(!visited[ikey]){
            visited[ikey]=true;
            hops.push({eid:ieid,fi:0,viaSplitter:true});
            queue.push({eid:ieid,fi:0});
          }
        }
      }
    }
  ...
  if(safety>=SAFETY_MAX){
    console.warn('[traceFiberGlobal] limite de '+SAFETY_MAX+' hops atingido — trajeto pode estar incompleto. Início:',startEid,'fi:',startFi);
  }
  return hops;
}

// Generaliza findOLTForFiber: retorna {tipo:'olt'|'mpls', nome, pon} ou null.
// Usado pra colorir/identificar fibras conectadas indiretamente (passagem).
function findSourceForFiber(cabId,fi){
  var seeds=['cable:'+cabId,'cable:'+cabId+'::I','cable:'+cabId+'::O'];
  for(var s=0;s<seeds.length;s++){
    var hops=traceFiberGlobal(seeds[s],fi);
    for(var i=0;i<hops.length;i++){
      var h=hops[i];
      if(h.eid.indexOf('out:')===0){
        var sid=h.eid.slice(4);
        var el=findById(sid);
        if(el&&(el.tipo==='olt'||el.tipo==='mpls')){
          return {tipo:el.tipo,nome:el.nome||el.id,pon:h.fi+1};
        }
      }
    }
  }
...
function showCableFiberPathModal(cabId,fi){
  ...
  if(hops.length===0){
    ...
    hops.forEach(function(h,i){
      ...
      html+='<div style="flex:1;font-size:12px"><b style="color:var(--text)">'+escH(d.tipo)+' '+escH(d.nome)+'</b><br><span style="color:var(--text2);font-family:var(--mono);font-size:11px">'+escH(d.detail)+'</span></div>';
      html+='</div>';
    });
  }
  html+='</div>';
...
// ═══════ SEGMENTOS DE FIBRA INDIVIDUAL ═══════
// Cada fibra pode ter emendas extras em pontos intermediários (anchors do cabo).
// cabo.fiberSegments = {"0":[3,7], "5":[3]} → fi 0 tem emendas nos anchors 3 e 7
function getFiberSplices(c,fi){
  if(!c||!c.fiberSegments)return [];
  var arr=c.fiberSegments[String(fi)];
  return Array.isArray(arr)?arr.slice().sort(function(a,b){return a-b;}):[];
}
function setFiberSplices(c,fi,anchorIdxs){
  if(!c)return;
  if(!c.fiberSegments)c.fiberSegments={};
  // Filtra: só anchors válidos do cabo
  var valid=(anchorIdxs||[]).filter(function(idx){
    return c.anchorMap&&c.anchorMap.hasOwnProperty(String(idx));
  }).filter(function(v,i,a){return a.indexOf(v)===i;}).sort(function(a,b){return a-b;});
  if(valid.length===0)delete c.fiberSegments[String(fi)];
  else c.fiberSegments[String(fi)]=valid;
}
// Calcula atenuação da fibra fi do cabo c incluindo emendas extras dos segmentos
function calcFiberAtt(c,fi){
  var base=calcAtt(c);
  var extra=getFiberSplices(c,fi).length;
  if(extra===0)return Object.assign({},base,{extraSplices:0});
  var addLoss=extra*ATT.emenda;
  return Object.assign({},base,{extraSplices:extra,tot:base.tot+addLoss,margin:base.margin-addLoss});
}
// Lista anchors INTERMEDIÁRIOS do cabo (exclui primeiro e último — não são "no meio")
function getIntermediateAnchors(c){
  if(!c||!c.anchorMap)return [];
  var entries=Object.keys(c.anchorMap).map(function(k){
    return {idx:parseInt(k,10),anchorId:c.anchorMap[k]};
  }).sort(function(a,b){return a.idx-b.idx;});
  if(entries.length<=2)return [];
  return entries.slice(1,-1); // remove primeiro e último
}
