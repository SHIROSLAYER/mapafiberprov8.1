...
function initMap(){
  ...
  map.on('mousemove', e => {
    ...
    // Preview cabo em traçado (reusa a polyline em vez de recriar a cada pixel)
    if(drawingCabo && caboPoints.length > 0) {
      const pts = caboPoints.concat([[e.latlng.lat, e.latlng.lng]]);
      if(caboPolyline){
        caboPolyline.setLatLngs(pts);
      }else{
    ...
    // Preview linha de medição (idem)
    if(mode==='medida' && measurePoints.length>0 && !measurePaused){
      const mpts=measurePoints.concat([[e.latlng.lat,e.latlng.lng]]);
      if(measurePreviewLine){
        measurePreviewLine.setLatLngs(mpts);
      }else{
        measurePreviewLine = L.polyline(mpts,
